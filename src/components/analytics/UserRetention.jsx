...
const userRetentionData = [
  { name: "Week 1", retention: 97 },
  { name: "Week 2", retention: 78 },
  { name: "Week 3", retention: 55 },
  { name: "Week 4", retention: 60 },
  { name: "Week 5", retention: 70 },
  { name: "Week 6", retention: 50 },
  { name: "Week 7", retention: 65 },
  { name: "Week 8", retention: 55 },
];


// Custom dot using happy/sad face SVGs
const CustomDot = (props) => {
  const { cx, cy, value } = props;

  const isHappy = value >= 60;
  const facePath = isHappy
  ...
  return (
    <svg x={cx - 10} y={cy - 10} width={20} height={20} viewBox="0 0 1024 1024" fill={isHappy ? "green" : "red"}>
      <path d={facePath} />
    </svg>
  );
};
